...
{
    class MyTestException : Exception  //自定义exception，实现3个ctor
    {
        public MyTestException() : base("It's a test exception.") { }
        public MyTestException(string s) : base(s) { }
        public MyTestException(string s,Exception inner):base(s,inner){ }
    }
    class Program
    {
            ...
            if (int.TryParse(b1, out input))
            {
                ...
                switch (input)
                {
                    ...
                    case 1:
                        ...
                        catch (IndexOutOfRangeException ex)//这个block要放在catch（Exception ex）前面，否则报错，因为明显Exception包含了所有其他错误。
                        {
